...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questions = postings.filter(_.postingType == QUESTION).keyBy(_.id)
    val answers = postings.filter(_.postingType == ANSWER).keyBy(_.parentId.get)
