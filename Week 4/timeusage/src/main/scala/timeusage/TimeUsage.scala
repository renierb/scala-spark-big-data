...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    val finalDf2Sql = timeUsageGroupedSql(summaryDf)
    finalDf2Sql.show()
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      Array(StructField(columnNames.head, StringType, nullable = false)) ++
  ...
  def row(line: List[String]): Row =
    Row(List(line.head) ++ line.tail.map(_.toDouble):_*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def columnsFor(prefixes: List[String]): List[Column] = {
      columnNames.filter(name => prefixes.exists(prefix => name.startsWith(prefix))).map(col)
    }
    ...
    val primaryColumns = columnsFor(primary)
    val workingColumns = columnsFor(working).diff(primaryColumns)
    val leisureColumns = columnsFor(leisure).diff(primaryColumns).diff(workingColumns)
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      ...
      .map {
        case (key, primaryNeeds, work, other) =>
          TimeUsageRow(key._1, key._2, key._3, round(primaryNeeds, 1), round(work, 1), round(other, 1))
      }
      .orderBy($"working", $"sex", $"age")
  }
}
